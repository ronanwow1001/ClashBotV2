...
describe('url', function() {
  ...
  it('*', function() {
    ...
    assert.ok(url.is('https://github.com'));
    assert.ok(!url.is('https://github'));

    assert.ok(url.match('https://github.com'));
    assert.ok(url.contain('https://github.com'));
  });

  it('match URLs', function() {
    urls.forEach(function (el) {
      if(!url.e().exec(el)) {
        assert.ok(false);
        return;
      }
      assert.ok(url.e().exec(el)[0] === el);
    });
  });

  it('do not match URLs', function() {
    noturls.forEach(function (el) {
      if(!url.e().exec(el)) {
        assert.ok(true);
        return;
      }
